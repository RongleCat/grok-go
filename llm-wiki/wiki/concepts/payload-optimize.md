...
`payload_optimize` + `files_api` 在转发前压缩请求体；**仅超大 tool 输出**可改成 **Files 上传 + `file_id` 引用**。  
**禁止**分流 message 里的 `input_text`（Codex `skills_instructions` / developer 指令常 >32KB；分流后注入 “use attachment_search” 会把 agent 带进找文件/乱调工具的死循环，见会话 `019f65cb`）。

## Claude Code / Anthropic 路径的额外预算

仅靠 **12–24 MiB 字节**预算不够：纯文本 agent 在 ~100k tokens 时 body 仍远小于 1 MiB，旧逻辑几乎不裁剪，结果是上游长 SSE 不稳定 → Claude Code `Connection closed mid-response`。

因此 `/v1/messages` 在 `optimize_responses_payload` 之后再跑 **`enforce_chat_context_budget`**：

| 阈值 | 行为 |
|---|---|
| 估算 token > 80k（soft） | 截断较旧 message/tool 文本 |
| 仍超 soft | 更狠截断近期 tool 输出 |
| > 100k（hard） | 压缩 tools description/schema 冗余 + 再截断 |
| 始终 | 按 128k 安全窗口 cap `max_tokens`（上限 16k） |

估算与 `count_tokens` 相同：~4 chars/token。日志：`chat context budget enforced`。
...
- **仅** `function_call_output` / `custom_tool_call_output` 且 ≥ 32KB：上传 xAI Files（账号级 content-hash 缓存）→ 输出改为中性 stub + 静默附带 `input_file.file_id`
- **不**分流 message `input_text`（skills / permissions / 用户正文保留；旧轮次靠同步路径历史截断）
- stub **不**提示 `attachment_search`（Codex 工具表无此工具，提示会诱发 shell 乱搜）
- 上传失败则回退 head+tail 截断，不阻断请求
