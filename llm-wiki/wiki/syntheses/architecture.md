...
5. `send_with_account_failover` 选号 → ensure refresh → 上游发送；401/403/429/5xx 可换号
6. 若存在 image_gen 类 tool，服务端 tool loop 调 Imagine 并回填
7. 响应侧把 function_call 回写为 custom_tool_call（如需）；写 usage 日志
8. 返回 JSON 或 SSE
